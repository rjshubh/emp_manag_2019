						...
						<div class="col-md-12">
							<div class="table-responsive">
								...
								<table class="table table-striped custom-table m-b-0 datatable">
									...
									<?php
									$i=0; 
									$query = ("SELECT * FROM revenue_planned ");
                    				...
                    				while ($row = mysqli_fetch_array($result1))
                    		{
        								...
        								echo '<input type = "submit" value= "update" name="address1'.$i.'" />';
        								 if (isset($_POST['address1'.$i.'']))
        								 {
        								 	if (isset($_POST['check1'.$i.'']))
        								 	{
        								 		...
        								 		$update= "update revenue_planned set ticketsale='$first', apr='$apr', may='$may', jun='$jun', jul='$jul',
        								...
        								if (isset($_POST['delete1'.$i.'']))
        								{
        									 ...
        									 {
        									 	$first= $_POST['tkt'.$i.''];
        									 	$delete = "delete from revenue_planned where ticketsale='$first'";
										...
										if(isset($_POST['add1']))
										{
												$first= $_POST['textfirst'];
        								 			...
        								 			is_null($oct) or is_null($nov) or is_null($dec) or is_null($jan) or is_null($feb) or is_null($mar) or is_null($total))
        								 		{
        											...
        											$add = "insert into revenue_planned values('$first','$apr','$may','$jun','$jul','$aug',
												...
												echo mysqli_error($db);}
        									 		...
        									 		///header("location:http://localhost/phpLab/emp_man/revenue.php");
        									 		//header( "Refresh: 0;" );
						...
						<div class="col-md-12">
							<div class="table-responsive">
								...
								<table class="table table-striped custom-table m-b-0 datatable">
									...
									<?php
									$i=0; 
									$query = ("SELECT * FROM revenue_actual ");
                    				...
                    				while ($row = mysqli_fetch_array($result))
                    		{
        								...
        								echo '<input type = "submit" value= "update" name="address'.$i.'" />';
        								 if (isset($_POST['address'.$i.'']))
        								 {
        								 	if (isset($_POST['check'.$i.'']))
        								 	{
        								 		...
        								 		$update= "update revenue_actual set ticketsale='$first', apr='$apr', may='$may', jun='$jun', jul='$jul',
        								 		aug='$aug', sep='$sep', oct='$oct', nov='$nov', dec2='$decmbr' ,jan='$jan', feb='$feb', mar='$mar', total='$total'
        								 		where  ticketsale='$first' ";
        								...
        								if (isset($_POST['delete'.$i.'']))
        								{
        									 ...
        									 {
        									 	$first= $_POST['tkt'.$i.''];
        									 	$delete = "delete from revenue_actual where ticketsale='$first'";
        									 	$qry = mysqli_query($db,$delete);
        									 	if (!$qry) {echo "deletion failed";}
        									 				//echo mysqli_error($db);
        									 	else{
										...
										if(isset($_POST['add2']))
										{
												$first= $_POST['textfirst'];
        								 			...
        								 			is_null($oct) or is_null($nov) or is_null($dec) or is_null($jan) or is_null($feb) or is_null($mar) or is_null($total))
        								 		{
        											...
        											$add = "insert into revenue_actual values('$first','$apr','$may','$jun','$jul','$aug',
        												'$sep','$oct','$nov','$dec','$jan','$feb','$mar','$total')";
													$qry = mysqli_query($db,$add);
													if (!$qry) {echo "adding failed";
												echo mysqli_error($db);}
